...
downwards_force = grav_f-downwards_force # POSITIVE MAGNITUDE OF FORCE. Includes gravity. Gravitational Force is always larger than or equal to.

# Normalize forces to slope angles.
forwards_force = (np.sin(slope) * downwards_force) + (np.cos(slope)*forwards_force) # Forwads backwards in relation to board direction.
downwards_force = (np.cos(slope)* -downwards_force) + (np.sin(slope)*forwards_force) # Up down direction
...
lowestY = FindY(-90) #Highest and lowest point where 0 is the binding point of the cone.
...
g_potent = grav_f * (highestY-lowestY) # Equivalent to mgh. Calculated relative to lowest point com will reach.
x,y = 0,0
...
for _ in range(int(graph_time/timestep )):
    ...
    purpendicular_vector = np.abs(np.sin(Theta)*forwards_force) # Force that goes into breaking static friction. Newtons.


    centrifugal = mass*length*com*(2 * np.pi * (a_velocity/com_circ))**2 # In Newtons, this force is directly purpendicular to the screw head.
    ...
    if not broke_Static:
        ...
        a_velocity -= ((parallel_vector)/mass)*timestep  # Acceleration multiplied by time to change the velocity. 

        if a_velocity != 0:
            direction = a_velocity/np.abs(a_velocity)

            a_velocity -= ((friction/rotational_fraction)/mass)*timestep*direction*2

        Theta += ((2*np.pi * (a_velocity/com_circ)))*timestep # Change Theta based upon the calculated angular velocity.

        if purpendicular_vector + centrifugal > grav_f * friction_coef * static_friction:
            broke_Static = True
    else:

        nonr_x = np.cos(Theta) * purpendicular_vector + centrifugal_x # The forces purpendicular to screw head put back into ramp space.
        nonr_y = np.sin(Theta) * purpendicular_vector + centrifugal_y

        angular_y = np.cos(Theta)*a_velocity # Forces that apply to the angular velocity
        ...
        if y_velocity != 0:
            ...
            y_velocity += np.sin(Theta) * ((friction*0.5)/mass) * timestep * direction
            #print(y_velocity<0)
        ...
        a_velocity -= ((parallel_vector)/mass)*timestep  # Acceleration multiplied by time to change the velocity. Metres per Second


        if a_velocity != 0: # Dampening Factor for Oscillation
            direction = a_velocity/np.abs(a_velocity)

            a_velocity -= ((friction/rotational_fraction)/mass)*timestep*direction
...
if angletime:
    ...
    def animate(i):
        print("Compiling:",math.floor((i/(graph_time*30*slowmotion))*1000)/10,"%")
        ...
        xdata,ydata = [(length*100*thread_position*x_vector)+Coordinates[1],(length*100*x_vector)+Coordinates[1]] , [graph_size*0.95+(length*100*thread_position*y_vector)+Coordinates[2],graph_size*0.95+(length*100*y_vector)+Coordinates[2]]


        xdatah,ydatah=[xdata[1] + np.sin(Frame)*(head_diam*50),xdata[1] - np.sin(Frame)*(head_diam*50)],[ydata[1] - np.cos(Frame)*(head_diam*50),ydata[1] + np.cos(Frame)*(head_diam*50)]
    ...
    anim = animation.FuncAnimation(fig, animate, init_func=init, 
                                frames=int(graph_time*30*slowmotion), interval=np.floor(1000/30), blit=True) 
