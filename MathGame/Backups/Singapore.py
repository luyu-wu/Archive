...
operators_available = ["ADD", "SUB", "MUL", "DIV"]
...
translation_layer = {"ADD": "+", "SUB": "-", "MUL": "x", "DIV": "÷"}
...
data = {
    "settings": {
        ...
        "operators_allowed": [
            ...
            "SUB",
            "MUL",
        ],  # Operands useable according to the blacklist whitelist rule
...
def DeepCopyArray(
    array,
):  # Quick function to ensure that when an array is destructively operated upon, the original array is not the same memory address of operated array.
    new_arr = []
    for i in array:
        new_arr.append(i)
    return new_arr
...
if path.exists(file_name):  # Attempting to load a non-existent file will throw an error
    ...
    data = dict(enumerate(np.load(file_name, allow_pickle=True).flatten(), 1))[
        1
    ]  # This method to convert a npy array to a dictionary was sourced online.
...
def stopgame():  # Write back to file after game has ended
    data["settings"] = (
        settings  # I'm not certain on whether if when creating the settings variable its the same memory address, or a clone is created as a seperate address, so I just sync them here.
    )
...
def OperatorWindow():
    ...
    def LocalizeButtonCall(var):
        button = False  # PLaceholder

        def Localized_Button():
            found = False

            # Small linear search function
            counter = 0
            for in_white in settings["operators_allowed"]:
                if in_white == var:
                    found = counter
                counter += 1
            if found:
                settings["operators_allowed"].remove(var)
                button.config(background="grey")
            else:
                settings["operators_allowed"].append(var)
    ...
    for i in operators_available:
        var = i
        LocalizeButtonCall(var)
...
def SettingsWindow():
    ...
    button = tk.Button(
        frame, relief=tk.RAISED, command=resetSettings, text="[Reset Settings]"
    )
    ...
    for SettingName in settings:
        ...
        SettingDT = str(type(settings[SettingName]))[
            8:-2
        ]  # The individiual settings data type
        ...
        if SettingDT == "bool":
            ...
            button_local = tk.Button(
                frame,
                relief=tk.RAISED,
                command=BooleanFlip,
                text="[" + str(settings[setting_Name]) + "]",
            )
...
def analyticsFind(operators=[], negatives=0, decimals=0, magnitude=0):
    ...
    for question in questions_data:
        ...
        for i in question["operands"]:
            ...
            if int(math.log10(i)) != magnitude - 1 and magnitude != 0:
                fits = False
            if (
                abs(decimal.Decimal(str(i).rstrip("0")).as_tuple().exponent) == decimals
                and decimals != 0
            ):
                fits = False
    ...
    return (
        math.floor(np.average(question_times) * 100) / 100,
        correct,
        incorrect,
        correct + incorrect,
        str(np.floor((correct / (correct + incorrect)) * 1000) / 10) + "%",
    )
...
viewing = (
    "all"  # Utilized within the Analytics Window for checking Current or not Current
)
...
def AnalyticsWindow():
    ...
    average_time_1, correct_1, incorrect_1, total_1, percentage_1 = analyticsFind(
        settings["operators_allowed"],
        settings["negatives"],
        settings["decimal_places"],
        settings["log_10"],
    )  # Analytics of currently applied settings.
    ...
    button = tk.Button(
        frame, relief=tk.RAISED, command=resetQuestions, text="[Wipe Data]"
    )
    button.pack(anchor="e")
    ...
    button_local = tk.Button(
        ...
        relief=tk.RAISED,
        command=Redraw,
        text="[View Current Setting/All Questions]",
    )
    button_local.pack(anchor="e")
...
text_lab = tk.Text(
    txt_edit, width=150, height=15, bg="white", fg="black", relief=tk.SUNKEN
)
...
sys_write = (
    sys.stdout.write
)  # in case I want to use terminal prints for debugging while keeping the application.
if not Debug_Mode:
...
def OperandGen():
    ...
    while integer == 0:
        if neg:
            integer = random.randint(-max_val, max_val) / (
                10**dec_val
            )  # Random Number between negative max and positive max

        else:
            integer = random.randint(0, max_val) / (
                10**dec_val
            )  # Random number between 0 and positive max
    return integer


def correct_decimals(
    num_string: str,
):  # Python generally prints numbers with a digit (e.g 5.0 instead of 5)
    if not clean_strings:
        return num_string
    while (
        num_string.endswith("0") and len(num_string) != 1 and num_string.find(".") != -1
    ):
...
def CalculateAnswer(values_memadd, ops_memadd):
    values = DeepCopyArray(
        values_memadd
    )  # Since this function is destructive on the parameters, I want to make sure that it's not operating on the arrays mem values within the class, so I made a unique mem_add cloned array
    ops = DeepCopyArray(ops_memadd)
    if (
        len(values) - len(ops) != 1
    ):  # There must always be exactly one more operand than operator as each operator must go between 2 operands.
    ...
    while not end:
        ...
        for i in ops:
            if i == "MUL" or i == "DIV":
                ...
                b = values[counter + 1]
                ops.pop(counter)
                values[counter] = operator_functions[i](
                    a, b
                )  # Operator function because it's easier than converting the string to a operator using 4 if/else statements.
    ...
    while not end:
        ...
        for i in ops:
            ...
            else:  # If it isn't addition of subtraction, that means another type of operator was in the mix which means the program is compromised.
                print("Critical Error; MUL/DIV OOO WHILE FAILED")
                exit()
            counter += 1
        if not broken:
            end = True
    ...
    solution = (
        math.floor(solution * (10 ** settings["decimal_places"]))
        / (10 ** settings["decimal_places"])
    )  # This makes it precisely the amount of deceimals in the decimal place specified value.
...
class Question:
    def __init__(self, values, ops):
        ...
        formed_equation = formed_equation + correct_decimals(
            str(values[len(values) - 1])
        )
...
def GenerateQuestion():
    ...
    ops_needed = settings["num_operands"]

    for i in range(ops_needed - 1):
        allowed = settings["operators_allowed"]
        random_op = allowed[random.randint(0, len(allowed) - 1)]
        operators.append(random_op)
...
def QuestionWindowThread():
    while True:
        ...
        questions_data.append(
            question.__dict__
        )  # Converts the Class into an array. NESSECARY due to JSON and saving complexities
        time.sleep(0.25)
