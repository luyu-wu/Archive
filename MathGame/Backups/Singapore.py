...
'settings':{
    ...
    'operators_allowed':["ADD","SUB","MUL"],         # Operands useable according to the blacklist whitelist rule
...
def OperatorWindow():
    ...
    def LocalizeButtonCall(var):
        ...
        button = tk.Button(newWindow,relief=tk.RAISED,text=var+"        ",command=Localized_Button,background='grey')  
        for in_white in settings['operators_allowed']:
            if in_white == var:
                button.config(background='white')
...
def AnalyticsWindow():
    ...
    average_time_1,correct_1,incorrect_1,total_1,percentage_1 = analyticsFind(settings['operators_allowed'],settings['negatives'],settings['decimal_places'],settings['log_10']) # Analytics of currently applied settings.
